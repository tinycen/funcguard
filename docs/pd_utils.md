...
| 功能模块 | 说明 | 详细文档 |
|----------|------|----------|
| **数据填充** | 空值填充 | [fill.md](./pandas/fill.md) |
| **日期时间** | NaT 时间填充、日期差值计算 | [date.md](./pandas/date.md) |
| **类型转换** | 列类型转换、Series 数值转换、Decimal 转换、日期时间转换、Series 格式转换、四舍五入 | [convert.md](./pandas/convert.md) |
| **JSON 处理** | JSON 字符串解析和转换 | [json.md](./pandas/json.md) |
| **数据筛选** | 智能条件筛选、列选择 | [filter.md](./pandas/filter.md) |
| **掩码构建** | 底层掩码构建函数 | [mask.md](./pandas/mask.md) |
| **条件计数** | 条件计数、不同值计数统计 | [count.md](./pandas/count.md) |
| **分组聚合** | 按列分组聚合统计（sum/mean/max/min等） | [agg.md](./pandas/agg.md) |
| **统计分析** | DataFrameStatistics 高性能统计分析类 | [statistics.md](./pandas/statistics.md) |
...
# 分组聚合统计：按部门统计工资总和
dept_salary = stats.group_agg("department", "salary", "sum")
print(f"各部门工资总和: {dept_salary}")

# 分组聚合统计：按部门统计平均工资
dept_avg = stats.group_agg("department", "salary", "mean")
print(f"各部门平均工资: {dept_avg}")
