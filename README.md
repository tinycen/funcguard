...
#### curl_cffi 兜底（绕过 403 反爬）

当目标网站启用反爬虫时，可启用 `curl_fallback` 自动使用 curl_cffi 重试：

```python
response = send_request(
    method="GET",
    url="https://example.com/protected",
    curl_fallback=True,  # 403 时自动使用 curl_cffi
    curl_fallback_impersonate="chrome124"  # 浏览器指纹
)
```

#### 检查 URL 有效性

使用 `check_url_valid` 快速检测 URL 是否可访问：

```python
from funcguard import check_url_valid

# 基础检测
is_valid = check_url_valid("https://example.com")
...
违禁词检测、文本清理、URL清理和年份归一化：

```python
from funcguard import check_text, clean_text, clean_url, TextCleaner
...
# 清理URL：去除查询参数与锚点
url = clean_url("https://detail.1688.com/offer/67764217.html?spm=a261y.7663282.3002526303362591.2.32671cfas8Yzdd&sk=order")
print(url)  # https://detail.1688.com/offer/67764217.html
...
| 函数/类名 | 功能说明 | 文档 |
|-----------|----------|------|
| `send_request` | HTTP请求封装（支持自动重试、curl_cffi兜底） | [查看](docs/network.md#send_request) |
| `curl_cffi_request` | 使用curl_cffi发送HTTP请求（TLS指纹伪装） | [查看](docs/network.md#curl_cffi_request) |
| `check_url_valid` | 检查URL是否有效 | [查看](docs/network.md#check_url_valid) |
| `md5_hash` | MD5哈希计算 | - |
...
| `pd_select_columns` | 列选择（列表或txt文件） | [查看](docs/pandas/filter.md) |
...
| `clean_url` | 清理 URL（去除查询参数与锚点） | [查看](docs/text_utils.md#clean_url) |
